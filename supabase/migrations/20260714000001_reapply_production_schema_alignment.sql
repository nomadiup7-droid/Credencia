...
select 'claim_' || encode(extensions.gen_random_bytes(8), 'hex'), c.event_id, c.id, coalesce(v.value->>'id', 'vol_1'),
...
declare
  ...
  v_next_tag integer;
  v_id text := coalesce(nullif(p_item->>'id', ''), 'c_' || encode(extensions.gen_random_bytes(8), 'hex'));
...
begin
  ...
  select 'claim_' || encode(extensions.gen_random_bytes(8), 'hex'), v_event_id, v_id, value->>'id', coalesce(nullif(value->>'storageRackId', ''), 'principal'), value->>'storageAddress', v_now, nullif(value->>'storageOperatorId', '')
